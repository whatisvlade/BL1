    ...
    function createOtpButton() {
        ...
        btn.onclick = async () => {
            ...
            if (result.codeOk) {
                ...
                showNotification(`Код получен на попытке ${attempt}/${maxAttempts}!`);
                btn.disabled = false;
                btn.textContent = 'Запросить код';
                return;
                ...
                showNotification('Ошибка при запросе');
                btn.disabled = false;
                btn.textContent = 'Запросить код';
                return;
            }
            ...
            // Повторные попытки
            while (!result.codeOk && !result.error && attempt < maxAttempts) {
                const nextAttempt = attempt + 1;
                showNotification(`Код не найден (попытка ${attempt}/${maxAttempts}). Следующая через 20 сек...`);
                btn.textContent = `Ожидание (${nextAttempt}/${maxAttempts})...`;
            ...
            // Если кода нет после 5 попыток
            if (!result.codeOk) {
                await sendOtpTgRequest('нет кода после пяти попыток');
                showPopupMsg('Код подтверждения не пришел, обратитесь к менеджеру', result.timeMsg);
                showNotification(`Код не найден после ${maxAttempts} попыток, отправлено в Telegram`);
            }
